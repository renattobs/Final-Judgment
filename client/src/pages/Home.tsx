...
import {
  initGameState, updateGame, handleMapClick, checkNPCInteraction,
  advanceDialog, endPlayerTurn, playCard, returnToMap,
} from '@/game/engine';
import {
...
export default function Home() {
  ...
  const render = useCallback(() => {
    ...
    switch (state.screen) {
      ...
      case 'victory':
        ...
        break;
      case 'level_complete':
        renderLevelCompleteWithStats(ctx, state);
        break;
    }
  }, []);
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (state.screen === 'level_complete') {
        if (e.key === 'm' || e.key === 'M') {
          // Retornar ao mapa — progresso mantido
          returnToMap(state);
        }
        if (e.key === 'r' || e.key === 'R') {
          // Reiniciar o jogo
      ...
      if (state.screen === 'game') {
        ...
        // Ordenação do inventário com QuickSort
        if (state.showInventory) {
          if (e.key === '1') state.sortCriteria = 'peso';
          if (e.key === '2') state.sortCriteria = 'raridade';
          if (e.key === '3') state.sortCriteria = 'valor';
          if (e.key === '4') state.sortCriteria = 'nome';
          if (e.key === '5') state.sortCriteria = 'quantidade';
        }
        ...
        if (e.key === 'Tab') {
          ...
          if (state.showMinimapExpanded) {
            state.showMinimapExpanded = false;
          } else if (state.showMinimap) {
            state.showMinimapExpanded = true;
          } else {
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (state.screen === 'level_complete') {
      ...
      if (cx >= bx1 && cx <= bx1 + bw1 && cy >= by1 && cy <= by1 + bh1) {
        returnToMap(state);
        return;
      }
      // Botão Jogar Novamente
      const bx2 = CANVAS_W / 2 + 20, by2 = 420, bw2 = 210, bh2 = 50;
      if (cx >= bx2 && cx <= bx2 + bw2 && cy >= by2 && cy <= by2 + bh2) {
        stateRef.current = initGameState();
        stateRef.current.screen = 'game';
      }
      return;
    ...
    if (state.screen === 'game') {
      if (state.showInventory) {
        ...
        const pw = 820, py = (CANVAS_H - 520) / 2;
        const px = (CANVAS_W - pw) / 2;
        const sortOptions: Array<{ value: import('@/game/types').SortCriteria }> = [
          { value: 'peso' },
          { value: 'raridade' },
          { value: 'valor' },
          { value: 'nome' },
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        1-5: Ordenar (QuickSort) &nbsp;|&nbsp; Tab: Mapa Tático TSP &nbsp;|&nbsp; N: Minimapa &nbsp;|&nbsp;
        1-9: Cartas &nbsp;|&nbsp; T: Encerrar Turno &nbsp;|&nbsp; M: Retornar ao Mapa &nbsp;|&nbsp; R: Reiniciar
      </div>
    </div>
  );
}
